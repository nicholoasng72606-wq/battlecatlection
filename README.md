...
- 根據三階、四階／超本能狀態自動顯示對應圖片
- 以彩色標籤區分「已開四階/超本」「未開」「暫無」
### 🔍 篩選功能
- 依 **稀有度**（全部／超激稀有／傳說稀有）篩選
- 依 **四階狀態**（全部／已四階或超本／未四階／無 lv60 強化）篩選
- 依 **三階狀態**（全部／已三階／未三階／暫無三階）篩選
- 依 **系列** 篩選，可只顯示單一系列的貓咪
- 揀選特定系列後，可額外開啟「👀 顯示未擁有」，一併檢視該系列尚未收集的貓咪（開啟時四階／三階篩選會自動鎖定為「全部」）
### 📊 收藏統計
- 分別統計「超激稀有」與「傳說稀有」的已四階/超本、未四階、已三階、未三階數量
- 自動計算四階完成度與三階完成度（暫無三階的貓咪不計入三階統計）
### 📈 系列收集進度
- 顯示各超激稀有系列的擁有比例進度條（傳說稀有因每系列僅一隻貓，故不顯示）
- 進度條依收集率變色（低／中／高），100% 集齊時會有金色閃爍特效
- 若已篩選特定系列，進度條只顯示該系列；若已篩選「傳說稀有」，進度條區塊會自動隱藏
### 🌙 深色模式
- 一鍵切換淺色／深色主題
- 未手動設定時，會自動跟隨系統的 light/dark 偏好
- 手動切換後會記住選擇，下次開啟自動套用
### 💾 本機自動儲存
- 每次成功解析資料後，會自動將輸入內容與結果儲存於瀏覽器本機（localStorage）
- 重新開啟頁面時自動載入上次資料，並顯示載入時間提示
- 可另外清除本機儲存記錄（與「清空輸入」為獨立操作，互不影響）
### 📷 匯出圖片
- 可將目前（篩選後）顯示的已擁有貓咪網格匯出為 PNG 圖片，方便分享
- 匯出圖片會依照目前的淺色／深色主題產生對應底色
- ⚠️ 由於圖片來源為跨網域資源，若瀏覽器（尤其 Safari）阻擋讀取，貓咪圖片可能無法完整匯出，此時建議改用瀏覽器內建截圖功能
### 📱 其他
- 響應式網格，電腦、手機皆可瀏覽
